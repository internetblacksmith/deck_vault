class CardSetsController < ApplicationController
  ...
  def import_delver_csv
    unless params[:csv_file].present?
      redirect_to card_sets_path, alert: "Please select a CSV file to import"
      return
    end
    ...
    unless file.original_filename.end_with?(".csv")
      redirect_to card_sets_path, alert: "Please upload a CSV file exported from Delver Lens"
      return
    end
    ...
    if result.success?
      ...
      message = "#{mode_text} #{result.imported} cards"
      message += " (#{result.foils_imported} foils)" if result.foils_imported > 0
      message += ", skipped #{result.skipped}" if result.skipped > 0

      if result.downloaded_sets.any?
        set_names = result.downloaded_sets.map { |s| s[:name] }
        message += ". Downloaded #{set_names.count} set(s): #{set_names.first(3).join(', ')}"
        message += "..." if set_names.count > 3
      end

      if result.missing_sets.any?
        message += ". Could not find sets: #{result.missing_sets.first(3).join(', ')}"
        message += "..." if result.missing_sets.count > 3
      end
